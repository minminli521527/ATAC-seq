...
######################## 2.peak信息 ###########################
peak_SRR5874658<-annotatePeak(peakSRR5874658,tssRegion = c(-2500,2500),TxDb = txdb,addFlankGeneInfo = T,flankDistance = 5000)
# 在R里打输出的对象peak_SRR5874658，下面Console会显示ChIPseq的位点落在基因组上什么样的区域，分布情况如何。
peak_SRR5874658
as.GRanges(peak_SRR5874658)
# 输出as.GRanges(peak_SRR5874658)，peak信息
write.csv (as.GRanges(peak_SRR5874658), file ="peak_SRR5874658.csv")



################## 3.观察peaks在染色体的分布 #####################
# covplot函数可以直接接受macs2-callpeak的输出bed文件进行画图。
pdf('peakdistribution_SRR5874658.pdf')
covplot(peakSRR5874658, weightCol="V5") 
dev.off()
...
tagMatrix <- getTagMatrix(peakSRR5874658,windows=promoter)
tagHeatmap(tagMatrix, xlim=c(-3000, 3000), 
           color="red")
#peakHeatmap函数只要给文件名，就可以直接出peaks的热图，是针对promoter区上下游的窗口进行绘图
pdf('peakHeatmap_SRR5874658.pdf')
peakHeatmap(peakSRR5874658,
            weightCol = NULL, 
            TxDb = txdb,
            upstream = 3000, downstream = 3000, 
...
#使用getTagMatrix函数，把peak比对到这个窗口，并生成矩阵，供分析、可视化
tagMatrix <- getTagMatrix(peakSRR5874658,windows=promoter)
pdf('plotAvgProf_SRR5874658.pdf')
...
peakAnno <-annotatePeak(peakSRR5874658, 
                        ...
                        verbose = TRUE)
# 6.1 饼图
pdf('plotAnnoPie_SRR5874658.pdf')
plotAnnoPie(peakAnno)
dev.off()
# 6.2 柱状分布图
pdf('plotAnnoBar_SRR5874658.pdf')
plotAnnoBar(peakAnno)
dev.off()
# 6.2 vennpie
pdf('vennpie_SRR5874658.pdf')
vennpie(peakAnno)
dev.off()
# 6.3 UpSetR
pdf('upsetplot_SRR5874658.pdf')
upsetplot(peakAnno)
...
pdf('plotDistToTSS_SRR5874658.pdf')
